...
private:
  ...
  RC plan_node(UpdateStmt *update_stmt, std::unique_ptr<LogicalNode> &logical_operator);
  RC plan_node(ExplainStmt *explain_stmt, std::unique_ptr<LogicalNode> &logical_operator);
};